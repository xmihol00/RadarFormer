...
if __name__ == "__main__":
    ...
    if model_cfg['type'] == 'CDC':
        ...
        from rodnet.models import Axial as RODNet
    elif model_cfg['type'] == 'T_RODNet':
        print('trodnet')
    else:
    ...
    or model_cfg['type'] == 'unetr_2d_res_interpolate'):
        in_chirps = len(radar_configs['chirp_ids'])
        rodnet = RODNet(in_channels=in_chirps, n_class=n_class_test, stacked_num=stacked_num,
                        mnet_cfg=config_dict['model_cfg']['mnet_cfg'],
                        dcn=config_dict['model_cfg']['dcn'],
        ...
        rodnet = RODNet(in_channels=in_chirps, n_class=n_class_test, stacked_num=stacked_num,
                        ...
                        patch_size = config_dict['model_cfg']['patch_size'], 
                        hidden_size = config_dict['model_cfg']['hidden_size'], 
                        num_hilo_layers = config_dict['model_cfg']['num_hilo_layers'],
                        num_base_layers = config_dict['model_cfg']['num_base_layers'],                
                        mlp_dim = config_dict['model_cfg']['mlp_dim'],
    ...
    for subset in seq_names:
        ...
        for iter, data_dict in enumerate(dataloader):
            ...
            total_count += 1
            print("Testing %s: frame %4d to %4d | Load time: %.4f | Inference time: %.4f | total count: %4d | Process time: %.4f" %
                  (seq_name, start_frame_id, end_frame_id, load_time, infer_time, total_count, proc_time))
